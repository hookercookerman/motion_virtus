#describe "Virtus::Equalizer", "#<<" do
  #before do
    #@name = "User"
    #@attributes = [:first_name].freeze
    #@object = Virtus::Equalizer.new(@name, @attributes)
    #@first_name = "John"
    #@last_name = "Doe"
    #@klass = Class.new { attr_accessor :first_name, :last_name }
    #@klass.send(:include, @object)
    #@instance = @klass.new.tap do |instance|
  ...
  #describe '#eql?' do
    #describe "when the objects are simular" do
      #before do
        ##@other = @instance.dup
      #end

      #it "add a key to the comparison" do
        ##lambda { @object << :last_name}.should.raise(Exception)
        ##lambda{@object << :last_name}.should.not.change {@instance.eql?(@other)}
      #end
    #end
